...
from .scoring import sigmoid, zscore_by_gene
    ...
    for _, row in sensor_prior.iterrows():
        ...
        for receiver in z.index:
            if sensor_frac.loc[receiver] < min_expr_frac:
                ...
                # receiver score 只保留 sensor 本身的表达和特异性，不再使用 import/usage score
...
def _empirical_pvalues(
    ...
    """
    计算经验 p 值（置换检验），完全基于 availability 流程
...
def run_cell_mesh(
    ...
    参数:
        ...
        random_state: 随机种子
        beta_sensor: 传感器表达对 receiver score 的权重
        beta_specificity: 传感器特异性对 receiver score 的权重
        ...
        missing_C_norm: 当代谢物没有消耗证据时的默认 C_norm 值，默认 0.41
        missing_E_norm: 当代谢物没有外排证据时的默认 E_norm 值，默认 0.75
    ...
    parameters = {
        ...
        "algorithm": "metabolite availability",
        "cell_type_key": cell_type_key,
        "sample_key": sample_key,
        "layer": layer,
        "min_expr_frac": min_expr_frac,
